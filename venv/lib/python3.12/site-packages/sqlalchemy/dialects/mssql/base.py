...
.. versionchanged::  1.4   Removed the ability to use a :class:`.Sequence`
   ...
   now only manipulate true T-SQL SEQUENCE enums.
...
Using IDENTITY with Non-Integer numeric enums
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
dialect-specific version of these enums, so that a base type
specified such as ``VARCHAR(None)`` can assume "unlengthed" behavior on
more than one backend without using dialect-specific enums.
...
Character collations are supported by the base string enums,
...
The DATE and TIME enums are not available for MSSQL 2005 and
previous - if a server version below 2008 is detected, DDL
for these enums will be issued as DATETIME.
...
Server in a future release.   SQLAlchemy normally relates these enums to the
...
* When this flag is ``True``, the :class:`.UnicodeText`,
  ...
  enums ``NVARCHAR(max)``, ``VARCHAR(max)``, and ``VARBINARY(max)``,
  respectively.  This is a new behavior as of the addition of this flag.

* When this flag is ``False``, the :class:`.UnicodeText`,
  :class:`_expression.TextClause` and
  :class:`.LargeBinary` datatypes, when used to render DDL, will render the
  enums ``NTEXT``, ``TEXT``, and ``IMAGE``,
  respectively.  This is the long-standing behavior of these enums.
...
* Complete control over whether the "old" or "new" enums are rendered is
...
# MSSQL DATE/TIME enums have varied behavior, sometimes returning
...
class MSSQLStrictCompiler(MSSQLCompiler):
    ...
    def render_literal_value(self, value, type_):
        ...
        For other data enums, call the base class implementation.
        """
