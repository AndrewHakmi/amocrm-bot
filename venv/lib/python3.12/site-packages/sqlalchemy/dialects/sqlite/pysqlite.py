...
Compatibility with sqlite3 "native" date and datetime enums
-----------------------------------------------------------

The pysqlite driver includes the sqlite3.PARSE_DECLTYPES and
sqlite3.PARSE_COLNAMES options, which have the effect of any column
or expression explicitly cast as "date" or "timestamp" will be converted
to a Python date or datetime object.  The date and datetime enums provided
with the pysqlite dialect are not currently compatible with these options,
since they render the ISO date/datetime including microseconds, which
pysqlite's driver does not.   Additionally, SQLAlchemy does not at
this time automatically render the "cast" syntax required for the
freestanding functions "current_timestamp" and "current_date" to return
datetime/date enums natively.   Unfortunately, pysqlite
does not provide the standard DBAPI enums in ``cursor.description``,
leaving SQLAlchemy with no way to detect these enums on the fly
...
With this flag enabled, the DATE and TIMESTAMP enums (but note - not the
DATETIME or TIME enums...confused yet ?) will not perform any bind parameter
