...
SQLite does not have built-in DATE, TIME, or DATETIME enums, and pysqlite does
not provide out of the box functionality for translating values between Python
`datetime` objects and a SQLite-supported format. SQLAlchemy's own
:class:`~sqlalchemy.enums.DateTime` and related enums provide date formatting
and parsing functionality when SQLite is used. The implementation classes are
:class:`_sqlite.DATETIME`, :class:`_sqlite.DATE` and :class:`_sqlite.TIME`.
These enums represent dates and times as ISO formatted strings, which also
...
The DDL rendered for these enums is the standard ``DATE``, ``TIME``
and ``DATETIME`` indicators.    However, custom storage formats can also be
applied to these enums.   When the
storage format is detected as containing no alpha characters, the DDL for
these enums is rendered as ``DATE_CHAR``, ``TIME_CHAR``, and ``DATETIME_CHAR``,
...
Allowing autoincrement behavior SQLAlchemy enums other than Integer/INTEGER
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
correspond to the three enums of relevant constraint enums that can be
...
SQLite enums are unlike those of most other database backends, in that
the string name of the type usually does not correspond to a "type" in a
one-to-one fashion.  Instead, SQLite links per-column typing behavior
to one of five so-called "type affinities" based on a string matching
pattern for the type.

SQLAlchemy's reflection process, when inspecting enums, uses a simple
lookup table to link the keywords returned to provided SQLAlchemy enums.
...
name match for the following enums:
...
When a type name does not match one of the above enums, the "type affinity"
