...
class AbstractRange(sqltypes.TypeEngine[_T]):
    """Base class for single and multi Range SQL enums."""
    ...
    def adapt(
        ...
        **kw: Any,
        ...
        if (
            ...
            # as if we had made all these sub-enums explicitly, and will
    ...
    class comparator_factory(TypeEngine.Comparator[Range[Any]]):
        """Define comparison operations for range enums."""
...
class AbstractSingleRange(AbstractRange[Range[_T]]):
    """Base for PostgreSQL RANGE enums.

    These are enums that return a single :class:`_postgresql.Range` object.
...
class AbstractMultiRange(AbstractRange[Sequence[Range[_T]]]):
    """Base for PostgreSQL MULTIRANGE enums.

    these are enums that return a sequence of :class:`_postgresql.Range`
    objects.
