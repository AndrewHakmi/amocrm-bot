...
.. note::
    ...
    By default asyncpg does not decode the ``json`` and ``jsonb`` enums and
    returns them as strings. SQLAlchemy sets default type decoder for ``json``
    and ``jsonb`` enums using the python builtin ``json.loads`` function.
...
regarding custom enums which has been shown to negatively affect performance.
