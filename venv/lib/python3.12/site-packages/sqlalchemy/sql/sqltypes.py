...
class HasExpressionLookup(TypeEngineMixin):
    ...
    These rules are currently used by the numeric, integer and date enums
...
class String(Concatenable, TypeEngine[str]):
    """The base for all string and character enums.
    ...
    def __init__(
        ...
        :param collation: Optional, a column-level collation for
          ...
          .. note::
            ...
            correct enums are used on the database.
...
class Numeric(HasExpressionLookup, TypeEngine[_N]):
    """Base for non-integer numeric enums, such as
    ...
    def __init__(
        ...
        :param decimal_return_scale: Default scale to use when converting
         ...
         database enums don't have a notion of "scale", so by default the
         float type looks for the first ten decimal places when converting.
         Specifying this value will override that length.  Types which
         do include an explicit ".scale" value, such as the base
         :class:`.Numeric` as well as the MySQL float enums, will use the
...
class Float(Numeric[_N]):
    """Type representing floating point enums, such as ``FLOAT`` or ``REAL``.
    ...
    def __init__(
        ...
        :param decimal_return_scale: Default scale to use when converting
         ...
         MySQL float enums, which do include "scale", will use "scale"
...
class Double(Float[_N]):
    """A type for double ``FLOAT`` floating point enums.
...
class DateTime(
    ...
    Date and time enums return objects from the Python ``datetime``
    module.  Most DBAPIs have built in support for the datetime
    module, with the noted exception of SQLite.  In the case of
    SQLite, date and time enums are stored as strings which are then
    ...
    def __init__(self, timezone: bool = False):
         ...
         **base date/time-holding type only**.   It is recommended
         ...
         date/time-holding enums distinct from the timezone-capable
...
class _Binary(TypeEngine[bytes]):
    """Define base behavior for binary enums."""
...
class LargeBinary(_Binary):
    ...
    def __init__(self, length: Optional[int] = None):
        ...
        :param length: optional, a length for the column for use in
          DDL statements, for those binary enums that accept a length,
...
class SchemaType(SchemaEventTarget, TypeEngineMixin):
    ...
    Supports enums that must be explicitly created/dropped (i.e. PG ENUM type)
    as well as enums that are complimented by table or schema level
    ...
    def _set_parent(self, parent, **kw):
        ...
        # "native" enums with a separate CREATE/DROP e.g. Postgresql.ENUM
    ...
    def _is_impl_for_variant(
        ...
        # for enums that have _variant_mapping, all the impls in the map
...
class Enum(String, SchemaType, Emulated, TypeEngine[Union[str, enum.Enum]]):
    ...
    def __init__(self, *enums: Union[str, Type[enum.Enum]], **kw: Any) -> None:
        ...
        :param metadata: Associate this type directly with a ``MetaData``
           object. For enums that exist on the target database as an
        ...
        :param schema: Schema name of this type. For enums that exist on the
    ...
    def as_generic(self, allow_nulltype=False):
        try:
            ...
            raise NotImplementedError(
                ...
                "is undefined for enums that inherit Enum but do not have "
                "an `enums` attribute."
            ) from None
...
class Interval(Emulated, _AbstractInterval, TypeDecorator[dt.timedelta]):
    ...
    operations on platforms which do not support interval enums natively. Such
    ...
    def __init__(
        ...
        :param second_precision: For native interval enums
          which support a "fractional seconds precision" parameter,
          i.e. Oracle Database and PostgreSQL

        :param day_precision: for native interval enums which
...
class JSON(Indexable, TypeEngine[Any]):
    ...
    .. note::  :class:`_types.JSON`
       ...
       JSON enums.  Since it supports JSON SQL operations, it only
    ...
    * Data casters for specific JSON element enums, subsequent to an index
    ...
    * :meth:`.JSON.Comparator.as_integer` - return the element as an integer
    ...
    assure that comparisons to the above enums will work as expected, such as::
...
class ARRAY(
    ...
    .. note::  This type serves as the basis for all ARRAY operations.
       ...
       ARRAY enums with PostgreSQL, as it provides additional operators
    ...
    * When used with the Python ``getitem`` accessor, the number of dimensions
    ...
    :meth:`.enums.ARRAY.Comparator.any` and
    :meth:`.enums.ARRAY.Comparator.all`. The PostgreSQL-specific version of
    ...
    def __init__(
        ...
        if isinstance(item_type, ARRAY):
            raise ValueError(
                "Do not nest ARRAY enums; ARRAY(basetype) "
    ...
    class Comparator(
        ...
        def any(
            ...
            .. seealso::
                ...
                :meth:`.enums.ARRAY.Comparator.all`
        ...
        def all(
            ...
            .. seealso::
                ...
                :meth:`.enums.ARRAY.Comparator.any`
...
class TIMESTAMP(DateTime):
    ...
    :paramref:`~enums.TIMESTAMP.timezone` argument in order to enable
...
class NullType(TypeEngine[None]):
    ...
    * When constructing SQL expressions using plain Python objects of
      unknown enums (e.g. ``somecolumn == my_special_object``)
...
class Uuid(Emulated, TypeEngine[_UUID_RETURN]):
    ...
    "native" mode enabled by default allows these enums will be used on those
    backends.
