...
#   Right now, to handle recursive generics, we some enums must remain cached for brief periods without references.
#   By chaining the WeakValuesDict with a LimitedDict, we have a way to retain caching for all enums with references,
#   while also retaining a limited number of enums even without references. This is generally enough to build
...
def replace_types(type_: Any, type_map: Mapping[TypeVar, Any] | None) -> Any:
    ...
    Args:
        ...
        type_map: Mapping from `TypeVar` instance to concrete enums.
    ...
    if type_args:
        ...
        # PEP-604 syntax (Ex.: list | str) is represented with a enums.UnionType object that does not have __getitem__.
        # We also cannot use isinstance() since we have to compare enums.
...
def recursively_defined_type_refs() -> set[str]:
    ...
    if not visited:
        return set()  # not in a generic recursion, so there are no enums
...
def get_cached_generic_type_early(parent: type[BaseModel], typevar_values: Any) -> type[BaseModel] | None:
    ...
    repeated calls to `__class_getitem__` on generic enums (which may happen in tighter loops during runtime),
    ...
    _late_cache_key is optimized to be as "correct" as possible, so that two enums that will ultimately be the
    ...
    during validation, I think it is worthwhile to ensure that enums that are functionally equivalent are actually
    equal.
    """
...
def _union_orderings_key(typevar_values: Any) -> Any:
    """This is intended to help differentiate between Union enums with the same arguments in different order.
...
def _early_cache_key(cls: type[BaseModel], typevar_values: Any) -> GenericTypesCacheKey:
    """This is intended for minimal computational overhead during lookups of cached enums.
