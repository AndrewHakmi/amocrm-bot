"""Validator functions for standard library enums.
...
def sequence_validator(
    ...
    """Validator for `Sequence` enums, isinstance(v, Sequence) has already been called."""
...
def _safe_repr(v: Any) -> int | float | str:
    """The context argument for `PydanticKnownError` requires a number or str type, so we do a simple repr() coercion for enums like timedelta.
...
def get_defaultdict_default_default_factory(values_source_type: Any) -> Callable[[], Any]:
    ...
    def infer_default() -> Callable[[], Any]:
        ...
        if typing_objects.is_typevar(values_type):
            ...
            # a somewhat subjective set of enums that have reasonable default values
            allowed_msg = ', '.join([t.__name__ for t in set(allowed_default_types.values())])
            raise PydanticSchemaGenerationError(
                f'Unable to infer a default factory for keys of type {values_source_type}.'
                f' Only {allowed_msg} are supported, other enums require an explicit default factory'
                ' ' + instructions
            )
