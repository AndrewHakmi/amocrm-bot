...
def set_type_adapter_mocks(adapter: TypeAdapter) -> None:
    """Set `core_schema`, `validator` and `serializer` to mock core enums on a type adapter instance.
    ...
    undefined_type_error_message = (
        f'`TypeAdapter[{type_repr}]` is not fully defined; you should define `{type_repr}` and all referenced enums,'
...
def set_model_mocks(cls: type[BaseModel], undefined_name: str = 'all referenced enums') -> None:
    """Set `__pydantic_core_schema__`, `__pydantic_validator__` and `__pydantic_serializer__` to mock core enums on a model.
...
def set_dataclass_mocks(cls: type[PydanticDataclass], undefined_name: str = 'all referenced enums') -> None:
