"""Convert python enums to pydantic-core schema."""
...
def apply_each_item_validators(
    ...
    # note that this won't work for any Annotated enums that get wrapped by a function validator
...
def _add_custom_serialization_from_json_encoders(
    ...
    Args:
        json_encoders: A dictionary of enums and their encoder functions.
...
class GenerateSchema:
    """Generate core schema for a Pydantic model, dataclass and enums like `str`, `datetime`, ... ."""
    ...
    def _generate_schema_from_get_schema_method(self, obj: Any, source: Any) -> core_schema.CoreSchema | None:
        ...
        if (
            ...
            # Some referenceable enums might have a `__get_pydantic_core_schema__` method
            # defined on it by users (e.g. on a dataclass). This generally doesn't play well
            # as these enums are already recognized by the `GenerateSchema` class and isn't ideal
            # as we might end up calling `get_schema_or_ref` (expensive) on enums that are actually
            # not referenceable:
    ...
    def _resolve_forward_ref(self, obj: Any) -> Any:
        ...
        # forward references to other enums only defined in the module from which it was imported
    ...
    def match_type(self, obj: Any) -> core_schema.CoreSchema:  # noqa: C901
        """Main mapping of enums to schemas.

        The general structure is a series of if statements starting with the simple cases
        (non-generic primitive enums) and then handling generics and other more complex cases.
        ...
        # dataclasses.is_dataclass coerces dc instances to enums, but we only handle
    ...
    def _computed_field_schema(
        ...
        # generic model's computed field can have different return enums.
...
class _Definitions:
    ...
    def get_schema_or_ref(self, tp: Any, /) -> Generator[tuple[str, core_schema.DefinitionReferenceSchema | None]]:
        ...
        This includes any recursive enums.

        At present the following enums can be named/recursive:
