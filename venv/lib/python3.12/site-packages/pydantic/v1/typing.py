...
if sys.version_info < (3, 9):
    ...
    def convert_generics(tp: Type[Any]) -> Type[Any]:
        ...
        if converted == args:
            ...
            # recreate enums.UnionType (PEP604, Python >= 3.10)
...
if sys.version_info < (3, 8):
    ...
    def is_none_type(type_: Any) -> bool:
        ...
        # can change on very subtle changes like use of enums in other modules,
...
def get_sub_types(tp: Any) -> List[Any]:
    """
    Return all the enums that are allowed by type `tp`
    `tp` can be a `Union` of allowed enums or an `Annotated` type
    """
    origin = get_origin(tp)
    if origin is Annotated:
