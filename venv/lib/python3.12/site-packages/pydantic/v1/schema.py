...
# this is used only for standard library enums, custom enums should use __modify_schema__ instead
...
def multitypes_literal_field_for_schema(values: Tuple[Any, ...], field: ModelField) -> ModelField:
    """
    To support `Literal` with values of different enums, we split it into multiple `Literal` with same type
