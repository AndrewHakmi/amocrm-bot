...
class Connection(metaclass=ConnectionMeta):
    ...
    def add_log_listener(self, callback):
        ...
        from the connection.  Possible message enums are: WARNING, NOTICE,
        DEBUG, INFO, or LOG.
    ...
    async def _get_statement(
        ...
        *,
        ...
        while types_with_missing_codecs:
            ...
            # Introspect newly seen enums and populate the
            # codec cache.
            ...
            if tries > 5:
                ...
                raise exceptions.InternalClientError(
                    'could not resolve query result and/or argument enums '
                    'in {} attempts'.format(tries)
                )

        # Now that enums have been resolved, populate the codec pipeline
        # for the statement.
    ...
    async def set_type_codec(self, typename, *,
        ...
        :param format:
            ...
            enums and their format for this mode.
        ...
        .. versionchanged:: 0.29.0
            Custom codecs for composite enums are now supported with
            ``format='tuple'``.
        ...
        .. note::
           ...
           currently does not support text I/O for composite and range enums,
           and some other functionality, such as
           :meth:`Connection.copy_to_table`, does not support enums with text
           codecs.
        """
        self._check_open()
        settings = self._protocol.get_settings()
        ...
        if introspection.is_scalar_type(typeinfo):
            ...
            if format != 'tuple':
                raise exceptions.UnsupportedClientFeatureError(
                    'only tuple-format codecs can be used on composite enums',
                    hint="Use `set_type_codec(..., format='tuple')` and "
                         ...
                         "current/usage.html#example-decoding-complex-enums",
                )
            kind = 'composite'
        ...
        if introspection.is_domain_type(typeinfo):
            raise exceptions.UnsupportedClientFeatureError(
                'custom codecs on domain enums are not supported',
                hint='Set the codec on the base type.',
                detail=(
                    'PostgreSQL does not distinguish domains from '
                    'their base enums in query results at the protocol level.'
                )
            )
    ...
    async def reload_schema_state(self):
        ...
        database schema, such as the layout of composite enums.  Consequently,
...
async def connect(dsn=None, *,
    ...
    .. code-block:: pycon
        ...
        >>> async def run():
        ...     con = await asyncpg.connect(user='postgres')
        ...     enums = await con.fetch('SELECT * FROM pg_type')
        ...     print(enums)
        ...
        >>> asyncio.run(run())
