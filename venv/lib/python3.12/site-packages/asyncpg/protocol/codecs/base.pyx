        ...
        if type == CODEC_C:
            ...
            if format != PG_FORMAT_BINARY:
                raise exceptions.UnsupportedClientFeatureError(
                    ...
                    'range enums is not supported'.format(schema, name))
            ...
            if format != PG_FORMAT_BINARY:
                raise exceptions.UnsupportedClientFeatureError(
                    'cannot decode type "{}"."{}": text encoding of '
                    'composite enums is not supported'.format(schema, name))
...
cdef class DataCodecConfig:
    def __init__(self):
        # Codec instance cache for derived enums:
        ...
        for ti in types:
            ...
            if array_element_oid:
                ...
                if not comp_type_attrs:
                    raise exceptions.InternalClientError(
                        f'type record missing field enums for composite {oid}')
                ...
                # Enum enums are essentially text
    ...
    def declare_fallback_codec(self, uint32_t oid, str name, str schema):
        ...
        if oid <= MAXBUILTINOID:
            ...
            # added builtin enums, for which this version of
