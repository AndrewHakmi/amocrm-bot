...
class BaseDAO(Generic[T]):
    ...
    async def find_one_or_none(self, filters: dict[str, Any] | None = None) -> Optional[T]:
        try:
            filters = filters or {}
            ...
            for field, value in filters.items():
                ...
                if isinstance(value, tuple) and len(value) == 2:
                    ...
                    conditions.append(OPS[op](column, operand))
                else:
            ...
            logger.exception(f"Error finding record with filters {filters}: {e}")
            raise
